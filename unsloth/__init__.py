...
# Check for unsloth_zoo (optional in CPU debug mode)
_has_unsloth_zoo = False
try:
    ...
    if not IS_CPU_DEBUG:
        ...
        print("Unsloth: unsloth_zoo not installed - some features are unavailable in CPU debug mode.")
except:
    if not IS_CPU_DEBUG:
        raise
    else:
        print("Unsloth: unsloth_zoo import failed - some features are unavailable in CPU debug mode.")
